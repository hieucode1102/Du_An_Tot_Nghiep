            ...
            <table class="table table-striped">
                ...
                <c:forEach items="${pageData.content}" var="st">
                    <tr>
                        ...
                        <td>${st.ma}</td>
                        <td>${st.ten}</td>
                        <td>${st.trangThai==1?"Kích hoạt":"Không kích hoạt"}</td>
                        <td><a href="/san-pham/edit/${st.ID}" class="btn btn-warning">Chỉnh sửa</a></td>
                        <td>
                            <a href="/san-pham/delete/${st.ID}" class="btn btn-danger" onclick="return confirm('Xác nhận xóa')">Xóa</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <c:if test="${pageData.number > 0}">
                        <li class="page-item">
                            <a class="page-link" href="/san-pham/index?page=${pageData.number - 1}">Previous</a>
                        </li>
                    </c:if>
                    ...
                    <c:if test="${pageData.number +1 < pageData.getTotalPages()}">
                        <li class="page-item">
                            <a class="page-link" href="/san-pham/index?page=${pageData.number + 1}">Next</a>
                        </li>
                    </c:if>
